    ...
    def company_search(self, input):
        ...
        if input == '*':
            result = list(OrgCompanies.objects.filter(client=global_variables.GLOBAL_CLIENT,
                                                      object_id_id__isnull=False,del_ind =False).values_list('object_id',flat=True))
        else:
            result = list(OrgCompanies.objects.filter(
                Q(client=global_variables.GLOBAL_CLIENT, object_id_id__isnull=False, company_id__icontains=input,del_ind =False) |
                Q(client=global_variables.GLOBAL_CLIENT, object_id_id__isnull=False,
                  name1__icontains=input,del_ind =False) | Q(client=global_variables.GLOBAL_CLIENT, object_id_id__isnull=False,
                                              name2__icontains=input,del_ind =False)).values_list('object_id',flat=True))
        org_model_result = list(OrgModel.objects.filter(client=global_variables.GLOBAL_CLIENT,
                                                        name__icontains=input,node_type=CONST_COMPANY_CODE,del_ind =False).values_list('object_id',flat=True))
        if org_model_result:
    ...
    def purchase_org_search(self, input):
        ...
        if input == '*':
            ...
            result = list(OrgPorg.objects.filter(
                Q(client=global_variables.GLOBAL_CLIENT, object_id_id__isnull=False, porg_id__icontains=input,del_ind =False) |
                Q(client=global_variables.GLOBAL_CLIENT, object_id_id__isnull=False,
                  description__icontains=input,del_ind =False)).values_list('object_id', flat=True))
        org_model_result = list(OrgModel.objects.filter(client=global_variables.GLOBAL_CLIENT,
                                                        name__icontains=input,
                                                        node_type=CONST_PORG,del_ind =False).values_list('object_id', flat=True))
        if org_model_result:
            result += org_model_result
        return result
    ...
    def purchase_grp_search(self, input):
        ...
        if input == '*':
            ...
            result = list(OrgPGroup.objects.filter(
                Q(client=global_variables.GLOBAL_CLIENT, object_id_id__isnull=False, pgroup_id__icontains=input,del_ind =False) |
                Q(client=global_variables.GLOBAL_CLIENT, object_id_id__isnull=False,
                  description__icontains=input,del_ind =False)).values_list('object_id', flat=True))
        org_model_result = list(OrgModel.objects.filter(client=global_variables.GLOBAL_CLIENT,
                                                        name__icontains=input,
    ...
    def org_user_search(self, input):
        ...
        if input == '*':
            ...
            result = list(UserData.objects.filter(
                Q(client=global_variables.GLOBAL_CLIENT, object_id_id__isnull=False, first_name__icontains=input,del_ind =False) |
                Q(client=global_variables.GLOBAL_CLIENT, object_id_id__isnull=False,
                  last_name__icontains=input,del_ind =False) | Q(client=global_variables.GLOBAL_CLIENT, object_id_id__isnull=False,
                                                  username__icontains=input,del_ind =False)).values_list('object_id', flat=True))
        org_model_result = list(OrgModel.objects.filter(client=global_variables.GLOBAL_CLIENT,
                                                        name__icontains=input,
